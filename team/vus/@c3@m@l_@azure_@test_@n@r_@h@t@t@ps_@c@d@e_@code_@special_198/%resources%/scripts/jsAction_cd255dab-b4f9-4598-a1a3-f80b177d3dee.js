var myClient = context.variableManager.getValue("list_client_product_quantity_NR_HTTPs_CDE_Code_Special_198.client");
if (myClient==null) {
...
else
{
        ...
        //context.variableManager.setValue("Current_Client",myClient);
        
        var myOrder = context.variableManager.getValue("list_client_product_quantity_NR_HTTPs_CDE_Code_Special_198.product_quantity");
        if (myOrder==null) {
        ...
        else
        {
                ...
                var myType = context.variableManager.getValue("list_client_product_quantity_NR_HTTPs_CDE_Code_Special_198.type");
                if (myType==null) {
                       context.fail(" Message type  not found, check input file");
                }
                
                var myAttendu = context.variableManager.getValue("list_client_product_quantity_NR_HTTPs_CDE_Code_Special_198.attendu_requete");
                if (myAttendu==null) {
                ...
                var myAttendu_ihm = context.variableManager.getValue("list_client_product_quantity_NR_HTTPs_CDE_Code_Special_198.attendu_IHM");
                if (myAttendu==null) {
                ...
                Body = Body + "<CSRP_ENVELOPPE xmlns=\"urn:x-csrp:fr.csrp.protocole:enveloppe\"  Version_Protocole=\"1.0.0.0\" Nature_Action=\"REQ_EMISSION\" Id_Logiciel=\"WinPharma\" Version_Logiciel=\"1.2.3.4\" Usage=\"P\">"+"\n";
                Body = Body + " <ENTETE>"+"\n";
               ...
               //No NORMALE node for request
                if (myType == "order"){
                             Body = Body + "  <SPECIALE Code_Speciale=\"198\">"+"\n";
                }
                ...
                for (var i = 0; i < productNUmber ; i++){
                    line_num = i+1;
                    if (myType == "order"){
                             Body = Body+ "  <LIGNE_S Num_Ligne=\""+line_num +"\" Type_Codification= \"CIP39\" Code_Produit=\""+orderarray[i*2]+"\" Quantite=\""+orderarray[i*2+1]+"\"></LIGNE_S>"+"\n";
                      }
                    if (myType == "request"){
